...
import uk.gov.hmrc.mobiletaxcreditsrenewal.domain.TaxSummaryDetails
...
class PersonalTaxSummaryConnectorSpec
  ...
  private trait Setup {
    ...
    val estimatedIncome = PTSEstimatedIncomeViewModel(graph = BandedGraph("1"), taxBands = None, ukDividends = None,
      incomeTaxReducedToZeroMessage = None, newGraph = BandedGraph("1"))
    val yourTaxableIncome = PTSYourTaxableIncomeViewModel(0, 0, 0, List(), None, EmploymentPension(None), List(), 0, List(), 0, List(), 0, List(), 0)
    ...
    class TestPersonalTaxSummaryConnector(http: CoreGet with CorePost,
                                          runModeConfiguration: Configuration,
                                          environment: Environment) extends PersonalTaxSummaryConnector(http, serviceUrl, runModeConfiguration, environment)
    ...
    val connector = new TestPersonalTaxSummaryConnector(http, mock[Configuration], mock[Environment])
  }
