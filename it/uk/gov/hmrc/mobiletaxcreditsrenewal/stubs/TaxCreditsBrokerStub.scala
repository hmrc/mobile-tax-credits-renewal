...
object TaxCreditsBrokerStub {
  ...
  def employedEarningsRtiFound(currentUserNino: Nino, withPartner: Boolean): Unit =
    stubFor(get(urlEqualTo(s"/tcs/${currentUserNino.value}/employed-earnings-rti"))
      .willReturn(
  ...
  def employedEarningsRtiError(currentUserNino: Nino, status: Int): Unit =
    stubFor(get(urlEqualTo(s"/tcs/${currentUserNino.value}/employed-earnings-rti"))
      .willReturn(
